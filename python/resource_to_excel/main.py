...
def get_instances_info(vpc_id, ec2):
    finishs = str(time.perf_counter())
    print('get_instances_infost  ' + finishs)
...
def lambda_handler(event, context):
    ...
    xlsx_name = 'vpc-prd-an2-mall-20200211.xlsx'
    # basic environment setting
    ...
    lambda_s = session.client('lambda')
    ...
    test = [outputbase64[i:i + length] for i in range(0, len(outputbase64), length)]
