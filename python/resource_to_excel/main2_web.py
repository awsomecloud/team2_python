...
def lambda_handler(event, context):
    ...
    event_json = {x.split('=')[0]: x.split('=')[1] for x in event['body'].split("&")}
    print('------event_json-----------')
    print(event_json['regionid'])
    print('------event_json-----------')
    print(event_json)

    region = event_json['regionid']
    #xlsx_name = event_json['filename']
    # access_key = event_json['akey']
    access_key = parse.unquote(event_json['akey'])
    secret_key = parse.unquote(event_json['skey'])

    #data_parameters = event['text'].split(' ')
    # basic environment setting
    # profile_name = data_parameters[2]
    session_token = ''
    tag_val = ['vpc-prd-an2-mall']
    xlsx_name = 'excel001.xlsx'
    # basic environment setting
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        #iam thread
        t_iam = executor.submit(get_iam_info, iam, 'n')
        # vpc thread
        ...
        # access key thread
        t_a_key = executor.submit(get_iam_info, iam, 'y')
...
#--------------------인스턴스 정보 Tab-----------------------------
    ...
    worksheet_ec2.write(ec2_start_row - 1, 0, '3.3 ALB 현황')
    worksheet_ec2.write(ec2_start_row, 0, '연번')
    ...
    header_result = {'statusCode': 200,
                     'body': outputbase64,
                     'headers':
                         {
                             'Set-Cookie': 'fileDownload=true; path=/',
                             'Content-Type': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
                             'Access-Control-Allow-Origin': 'https://excel.mzc-poc.be',
